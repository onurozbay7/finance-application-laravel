...
@section('content')
    <div class="row page-title clearfix">
        <div class="page-title-left">
            <h6 class="page-title-heading mr-0 mr-r-5">Kullanıcı <small> Kullanıcı Düzenle</small></h6>
        </div>
        <!-- /.page-title-left -->
                        ...
                        <form action="{{ route('user.update',['id'=>$data[0]['id']]) }}" autocomplete="off" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group row ">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input class="form-control" required name="email" value="{{ $data[0]['email'] }}"  type="email">
                                </div>

                                <div class="col-md-12">
                                    <label class="col-form-label" for="l0">Kullanıcı Şifre</label>
                                    <input class="form-control" placeholder="Değiştirmek istemiyorsanız lütfen boş bırakın."  name="password"  type="password">
                                </div>
                            </div>

                            <div class="col-md-12">
                            <label class="col-form-label" for="l0">Kullanıcı Yetkileri</label>
                            <div class="row col-md-3">

                                @foreach(\Illuminate\Support\Facades\Config::get('app.permissions') as $k => $v)
                                    <div class="col-md-4 mt-2 mb-2">
                                        <input id="{{ $v }}" @if(\App\Models\UserPermission::getControl($data[0]['id'],$k)) checked @endif type="checkbox" name="permission[]" value="{{ $k }}" >
