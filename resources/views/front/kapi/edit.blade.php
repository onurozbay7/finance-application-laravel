...
@section('content')
    <div class="row page-title clearfix">
        <div class="page-title-left">
            <h6 class="page-title-heading mr-0 mr-r-5">Ürün <small> Kapı Düzenle</small></h6>
            ...
            <div class="col-md-12 widget-holder">
                <div class="widget-bg">
                    <div class="widget-body clearfix">
                        ...
                        <form action="{{ route('kapi.update',['id'=>$data[0]['id']]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="col-md-12">
                                <div class="table-responsive">
                                    <table id="urunData" class="table">
                                        ...
                                        <tr class="kapi_field">
                                            <td><input type="text" class="form-control" required name="kapi[{{ $k }}][boy]" value="{{$v['boy']}}"></td>
                                            <td><input type="text" class="form-control" required name="kapi[{{ $k }}][en]" value="{{$v['en']}}"></td>
                                            <td><input type="text" class="form-control" required name="kapi[{{ $k }}][kasa]" value="{{$v['kasa']}}"></td>
                                            <td><input type="text" class="form-control" required name="kapi[{{ $k }}][adet]" value="{{$v['adet']}}"></td>
                                            <td><input type="text" class="form-control" name="kapi[{{ $k }}][kenar]" value="{{$v['kenar']}}"></td>
                                            <td><input type="text" class="form-control" name="kapi[{{ $k }}][model]" value="{{$v['model']}}"></td>
                                            <td><input type="text" class="form-control" name="kapi[{{ $k }}][renk]" value="{{$v['renk']}}"></td>
                                            <td><input type="text" class="form-control" name="kapi[{{ $k }}][text]" value="{{$v['text']}}"></td>
                                        </tr>
                                        @endforeach
                                    </table>
                                </div>
