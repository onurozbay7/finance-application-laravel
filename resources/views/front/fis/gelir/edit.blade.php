...
@section('content')
    <div class="row page-title clearfix">
        <div class="page-title-left">
            <h6 class="page-title-heading mr-0 mr-r-5">Fiş <small> Gelir Fişi Düzenle</small></h6>
            ...
            <div class="col-md-12 widget-holder">
                <div class="widget-bg">
                    <div class="widget-body clearfix">
                        ...
                        <form action="{{ route('fis.update',['id'=>$data[0]['id']]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                                    ...
                                    <table id="faturaData" class="table">
                                        ...
                                        @foreach($dataIslem as $k => $v)
                                            <tr class="islem_field">
                                                <td><select required name="islem[{{ $k }}][urun]" style="width: 100%" class="form-control" data-placeholder="Mdf veya Hırdavat Seçiniz" data-toggle="select2">
                                                        <option value="0">Ürünsüz Fiş Girişi (Stoktan Düşmez)  </option>
                                                        @foreach(\App\Models\Urun::all() as $key => $value)
                                                            <option @if($value['id'] == $v['urunId']) selected @endif value="{{$value['id']}}">{{ $v['urunTipi'] }} | {{ $value['text'] }} | {{ $value['stok'] }}</option>
                                                        @endforeach
                                                    </select></td>
                                                <td><input type="text" class="form-control" id="text" name="islem[{{ $k }}][text]" value="{{ $v['text'] }}"></td>
                                                <td><input type="text" class="form-control" id="adet" name="islem[{{ $k }}][adet]" value="{{ $v['miktar'] }}"></td>
                                                ...
                                                @if($k!=0) <td><button id="removeButton" type="button" class="btn btn-danger">X</button></td> @endif
                                            </tr>
        ...
        $(document).ready(function () {
            ...
            $("#addRowBtn").click(function (){
                ...
                var newRow =
                    ...
                    '<td><input required type="text" class="form-control" id="adet" name="islem['+i+'][adet]"></td>'+
                    '<td><input required type="text" class="form-control" id="tutar" name="islem['+i+'][tutar]"></td>'+
                    '<td><input required type="text" class="form-control" id="toplam_tutar" name="islem['+i+'][toplam_tutar]"></td>'+
                    '<td><input required type="text" class="form-control" id="kdv" name="islem['+i+'][kdv]"></td>'+
                    '<td><input required type="text" class="form-control" id="kdv_toplam" name="islem['+i+'][kdv_toplam]"></td>'+
                    '<td><input required type="text" class="form-control" id="genel_toplam" name="islem['+i+'][genel_toplam]"></td>'+
